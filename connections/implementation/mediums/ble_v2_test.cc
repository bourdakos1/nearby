...
constexpr absl::Duration kLostAlarmDuration = absl::Milliseconds(3000);
...
TEST_F(BleV2Test, CanStartAdvertising) {
  ...
  EXPECT_TRUE(ble_a.StartAdvertising(std::string(kServiceIDA),
                                     advertisement_bytes,
                                     PowerLevel::kHighPower, ""));
  EXPECT_TRUE(found_latch.Await(kWaitDuration).result());
  EXPECT_TRUE(ble_a.StopAdvertising(std::string(kServiceIDA)));
...
TEST_F(BleV2Test, StartFastScanningDiscoverAndLostPeripheral) {
  ...
  SystemClock::Sleep(kLostAlarmDuration * 2);
